...
async function pollingTimeout() {
    await sleep(POLLING_TIMEOUT_MS);
...
async function loopWithBackoff(pollingIntervalMs, fn) {
    ...
    while (true) {
        ...
        console.debug(`Loop time: ${loopDurationMs}ms, delay time: ${delayTimeMs}ms, polling interval: ${pollingIntervalMs}ms`);
        ...
        try {
            ...
            console.error('Error running sdk agent', error);
...
class RetoolAPI {
    ...
    async popQuery(options) {
        return fetch__default["default"](`${this._hostUrl}/api/v1/retoolsdk/popQuery`, {
            method: 'POST',
            headers: {
    ...
    async registerAgent(options) {
        return fetch__default["default"](`${this._hostUrl}/api/v1/retoolsdk/registerAgent`, {
            method: 'POST',
            // TODO: Add ApiToken
            headers: {
                Authorization: `Bearer ${this._apiKey}`,
    ...
    async postQueryResponse(options) {
        return fetch__default["default"](`${this._hostUrl}/api/v1/retoolsdk/postQueryResponse`, {
            method: 'POST',
            // TODO: add ApiToken
            headers: {
                Authorization: `Bearer ${this._apiKey}`,
...
class RetoolSDK {
    ...
    constructor(config) {
        ...
        this._logger.debug('Retool SDK Configuration', {
            apiKey: this._apiKey,
     ...
     */
    async listen() {
        this._logger.info('Starting SDK agent');
        const registerResult = await loopWithBackoff(this._pollingIntervalMs, () => this.registerAgent());
        if (registerResult === 'done') {
            this._logger.info('Agent registered');
            this._logger.info('Starting processing query');
            loopWithBackoff(this._pollingIntervalMs, () => this.fetchQueryAndExecute());
        }
    }
    /**
    ...
    async executeFunction(functionName, args, context) {
        ...
        if (functionName === '__testConnection__') {
            return { result: this.testConnection(context), args: {} };
        }
        ...
        this._logger.debug('Parsed arguments: ', parsedArgs);
    ...
    async registerAgent() {
        ...
        for (const functionName in this._functions) {
            functionsMetadata[functionName] = {
                ...
                access: this._functions[functionName].access,
            };
        }
    ...
    async fetchQueryAndExecute() {
        const popQueryResponsePromise = this._retoolApi.popQuery({
            resourceId: this._resourceId,
            environmentName: this._environmentName,
            agentUuid: this._agentUuid,
            versionHash: this._versionHash,
        });
        const pendingQueryFetch = await Promise.race([popQueryResponsePromise, pollingTimeout()]);
        if (!pendingQueryFetch) {
            throw new Error('Did not receive a query within the polling timeout');
        }
        ...
        if (query) {
            this._logger.debug('execute query', query); // This might contain sensitive information
